...
## PERSISTENCIA Y NAVEGACION DE FECHAS (5-sep-2026)
Reporte del dueño: al refrescar la página no se veían los análisis ya hechos, y al saltar de día no se podía volver atrás.
- **El tablero se lee de Supabase al abrir la página** (`cargarGuardado`), no solo de la memoria de la sesión. `desdeFilas` reconstruye cada candidato desde su objeto de decisión guardado, así que la tarjeta se ve igual que cuando se generó.
- **Barra de fechas**: día anterior, día siguiente, campo de fecha, botón Hoy y un desplegable con las fechas que ya tienen análisis. Moverse entre fechas no gasta ni una llamada a API-Football.
- **Aviso de origen**: el tablero dice si lo que se ve es guardado (con la hora de la última corrida) o en vivo.
